...
def _msat_make_minimize(e, term, lower, upper):
    """_msat_make_minimize(msat_env e, msat_term term, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_minimize(e, term, lower, upper)

def _msat_make_minimize_signed(e, term, lower, upper):
    """_msat_make_minimize_signed(msat_env e, msat_term term, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_minimize_signed(e, term, lower, upper)

def _msat_make_maximize(e, term, lower, upper):
    """_msat_make_maximize(msat_env e, msat_term term, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_maximize(e, term, lower, upper)

def _msat_make_maximize_signed(e, term, lower, upper):
    """_msat_make_maximize_signed(msat_env e, msat_term term, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_maximize_signed(e, term, lower, upper)

def _msat_make_minmax(e, len, terms, lower, upper):
    """_msat_make_minmax(msat_env e, size_t len, msat_term terms, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_minmax(e, len, terms, lower, upper)

def _msat_make_minmax_signed(e, len, terms, lower, upper):
    """_msat_make_minmax_signed(msat_env e, size_t len, msat_term terms, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_minmax_signed(e, len, terms, lower, upper)

def _msat_make_maxmin(e, len, terms, lower, upper):
    """_msat_make_maxmin(msat_env e, size_t len, msat_term terms, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_maxmin(e, len, terms, lower, upper)

def _msat_make_maxmin_signed(e, len, terms, lower, upper):
    """_msat_make_maxmin_signed(msat_env e, size_t len, msat_term terms, msat_term lower, msat_term upper) -> msat_objective"""
    return _mathsat._msat_make_maxmin_signed(e, len, terms, lower, upper)
...
def msat_make_minimize(env, term, lower=None, upper=None, signed=False):
    """
    Creates a new objective 'min(term)' with optional minimization
    ...
    ub = upper
    if lb is None:
        lb = MSAT_MAKE_ERROR_TERM()
    if ub is None:
        ub = MSAT_MAKE_ERROR_TERM()
    if signed:
        return _msat_make_minimize_signed(env, term, lb, ub)
    else:
        return _msat_make_minimize(env, term, lb, ub)

def msat_make_maximize(env, term, lower=None, upper=None, signed=False):
    """
    Creates a new objective 'max(term)' with optional maximization
    interval ]lower, upper].
    ...
    if ub is None:
        ub = MSAT_MAKE_ERROR_TERM()
    if signed:
        return _msat_make_maximize_signed(env, term, lb, ub)
    else:
        return _msat_make_maximize(env, term, lb, ub)

def msat_make_minmax(env, terms, lower=None, upper=None, signed=False):
    """
    Creates a new objective 'min(max(terms[0]), ..., max(terms[N]))' with
    optional optimization interval ]lower, upper].
    ...
    if ub is None:
        ub = MSAT_MAKE_ERROR_TERM()
    if signed:
        return _msat_make_minmax_signed(env, len(terms), terms, lb, ub)
    else:
        return _msat_make_minmax(env, len(terms), terms, lb, ub)

def msat_make_maxmin(env, terms, lower=None, upper=None, signed=False):
    """
    Creates a new objective 'max(min(terms[0]), ..., min(terms[N]))' with
    optional optimization interval [lower, upper[.
    ...
    if ub is None:
        ub = MSAT_MAKE_ERROR_TERM()
    if signed:
        return _msat_make_maxmin_signed(env, len(terms), terms, lb, ub)
    else:
        return _msat_make_maxmin(env, len(terms), terms, lb, ub)
...
def msat_objective_value_is_unbounded(env, obj, val):
    ...
    :param val:
        ...
        MSAT_INITIAL_LOWER: the initial lower bound.
        MSAT_INITIAL_UPPER: the initial upper bound.
        MSAT_FINAL_LOWER: the final lower bound.
        MSAT_FINAL_UPPER: the final upper bound.
        MSAT_FINAL_ERROR: the final absolute error.
        MSAT_FINAL_TOLERANCE: the final tolerance error.
